...
void add_grupo(Mesa **mesas, Quantidade qt_mesas, Fila *fila_espera, Pilha *pilha_pratos)
{
    ...
    while (gp.num_pessoas > 0)
    {
        ...
        else
        {
            ...
            {
                reserva_mesa(gp.num_pessoas, mesa_do_gp, pilha_pratos);
            }
            else
            {
                reserva_mesa(MAX_PESSOAS, mesa_do_gp, pilha_pratos);
            }
        }
    ...
    for (int i = 0; i < qt_mesas.lin; i++)
    {
        for (int j = 0; j < qt_mesas.col; j++)
        {
            if (mesas[i][j].ocupada == false)
            {
                ...
                {
                    reserva_mesa(fila_espera->ini->grupo.num_pessoas, mesa_do_gp, pilha_pratos);
                }
                else
                {
                    reserva_mesa(MAX_PESSOAS, mesa_do_gp, pilha_pratos);
